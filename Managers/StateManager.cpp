...
void StateManager::addCurrentState(State* state)
{
...
void StateManager::changeState(std::string stateName)
{
    ...
    else
    {
        ...
        {
            addCurrentState(static_cast<State*>(new MenuInicialState()));
        }
        else if(stateName == "GameState")
        {
            addCurrentState(static_cast<State*>(new FaseState("../Mapa/tiled/Fase1_1.tmj")));
        }
        else if(stateName == "MenuPauseState")
        {
            addCurrentState(static_cast<State*>(new MenuPauseState()));
        }
        else if(stateName == "MenuOptionsState")
        {
            addCurrentState(static_cast<State*>(new MenuOptionsState()));
        }
        else if(stateName == "MenuRankingState")
        {
            addCurrentState(static_cast<State*>(new MenuRankingState()));
        }
    }
}